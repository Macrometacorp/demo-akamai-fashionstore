...
const setCustomerId = (customerId: string) => sessionStorage.setItem(CUSTOMER_ID, customerId)
...
const API = {
    ...
    post: async function (key: string, path: string, data: any) {
        return await await fetchWrapper(path, getOptions({ method: "POST", body: JSON.stringify(data.body) }))
    },
    put: async function (key: string, path: string, data: any) {
        return await fetchWrapper(path, getOptions({ method: "PUT", body: JSON.stringify(data.body) }))
    },
    del: async function (key: string, path: string, data: any) {
        return await fetchWrapper(path, getOptions({ method: "DELETE", body: JSON.stringify(data.body) }))
    },
}
