    ...
    switch (queryName) {
        ...
        case "ListFashionItems":
            ...
            if (typeof bindValue === "object" && Object.keys(bindValue).length) {
                queryObj = {
                    query: "FOR fashionItem IN FashionItemsTable filter fashionItem.category == @category RETURN fashionItem",
                    bindVars: bindValue,
                }
            }
            break
        case "GetFashionItem":
            queryObj = {
                query: "FOR fashionItem in FashionItemsTable FILTER fashionItem._key == @fashionItemId RETURN fashionItem",
        ...
        case "UpdateCart":
            queryObj = {
                query: 'UPDATE {_key: CONCAT_SEPARATOR(":", @customerId, @fashionItemId),quantity: @quantity} IN CartTable',
                bindVars: bindValue,
            }
            break
        case "RemoveFromCart":
            queryObj = {
                query: 'REMOVE {_key: CONCAT_SEPARATOR(":", @customerId, @fashionItemId)} IN CartTable',
                bindVars: bindValue,
            }
            break
        case "GetCartItem":
            queryObj = {
                query: "FOR item IN CartTable FILTER item.customerId == @customerId AND item.fashionItemId == @fashionItemId RETURN item",
                bindVars: bindValue,
            }
            break
        case "ListOrders":
            queryObj = {
                query: "FOR item IN OrdersTable FILTER item.customerId == @customerId RETURN item",
                bindVars: bindValue,
            }
            break
        case "Checkout":
