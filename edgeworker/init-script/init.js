...
const COLLECTIONS = [
    {
        ...
        data: [
            ...
            },
            {
                _key: "f29",
                category: "Sportswear",
                name: "Fitg18® Gym wear Leggings Ankle Length Free Size Combo Workout Trousers",
                price: 22.99,
                rating: 3.8,
            },
            {
...
async function init(client) {
    ...
    for (view of VIEWS) {
        ...
        const exists = existingViews.find((existingView) => existingView.name === name)
        if (exists) {
    ...
    try {
        ...
        const streamAppExists = existingStreamApps.find((streamApp) => streamApp.name === STREAM_APP_NAME)
        if (!streamAppExists) {
            ...
            console.log(`Streamapp ${STREAM_APP_NAME} already exists. Skipping creation.`)
        }
    } catch (e) {
        console.log("Error!!!")
        console.log(e)
